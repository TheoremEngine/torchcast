...
class NEONAquaticDataset(TensorSeriesDataset):
    def __init__(self, path: str, download: Union[bool, str] = False,
        ...
        '''
        dates, tensor, sites = _get_neon_dataset(
            path,
            NEON_AQUATIC_URL,
            NEON_AQUATIC_FILE_NAME,
            NEON_AQUATIC_KEYS,
            download=download
        )
...
class NEONTerrestrialDataset(TensorSeriesDataset):
    def __init__(self, path: str, download: Union[bool, str] = False,
                 transform: Optional[Callable] = None,
                 return_length: Optional[int] = None):
        '''
...
def _get_neon_dataset(path: str, url: str, file_name: str, keys: List[str],
    ...
    var_ids = {k: i for i, k in enumerate(keys)}
    ...
    buff = torch.full(
        (len(site_ids), len(keys), n_t), float('nan'), dtype=torch.float32
    )
