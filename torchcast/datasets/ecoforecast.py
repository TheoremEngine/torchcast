...
class NEONAquaticDataset(TensorSeriesDataset):
    def __init__(self, path: str, download: Union[bool, str] = False,
        ...
        if os.path.isdir(path):
            path = os.path.join(path, NEON_AQUATIC_FILE_NAME)

        if not os.path.exists(path) or (download == 'force'):
            if download:
                path = _download_and_extract(
                    NEON_AQUATIC_URL, path, file_name=NEON_AQUATIC_FILE_NAME
                )
            else:
                raise FileNotFoundError(
                    f'NEON aquatic dataset not found at: {path}'
                )
        ...
        var_ids = {k: i for i, k in enumerate(NEON_AQUATIC_KEYS)}
        ...
        buff = torch.full(
            (len(site_ids), 3, n_t), float('nan'), dtype=torch.float32
        )
